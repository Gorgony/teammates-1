...
public class InstructorFeedbackSessionsPageAction extends InstructorFeedbackAbstractAction {
    ...
    protected ActionResult execute() {
        ...
        String courseIdForNewSession = getRequestParamValue(ParamNameConst.ParamsNames.COURSE_ID);
        String feedbackSessionToHighlight = getRequestParamValue(ParamNameConst.ParamsNames.FEEDBACK_SESSION_NAME);
        ...
        if (courses.isEmpty() || !data.isUsingAjax()) {
            ...
            if (existingFeedbackSessions.isEmpty()) {
                statusToUser.add(new StatusMessage(StatusMessageConst.StatusMessages.FEEDBACK_SESSION_EMPTY, StatusMessageColor.WARNING));
            }
        }

        if (courses.isEmpty()) {
            statusToUser.add(new StatusMessage(StatusMessageConst.StatusMessages.COURSE_EMPTY_IN_INSTRUCTOR_FEEDBACKS
