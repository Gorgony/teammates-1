...
<div class="panel-body">
  <form method="post" action="<%=Const.ActionURIs.INSTRUCTOR_COURSE_INSTRUCTOR_EDIT_SAVE%>"
    ...
    <input type="hidden" name="<%=ParamNameConst.ParamsNames.COURSE_ID%>" value="${instructorPanel.instructor.courseId}">
    <c:if test="${not empty instructorPanel.instructor.googleId}" >
      <input type="hidden" name="<%=ParamNameConst.ParamsNames.INSTRUCTOR_ID%>" value="${instructorPanel.instructor.googleId}">
    </c:if>
    ...
    <div id="instructorTable${instructorPanel.index}">
          ...
          <c:choose>
            <c:when test="${not empty instructorPanel.instructor.googleId}">
              <input class="form-control immutable"
                  type="text" id="<%=ParamNameConst.ParamsNames.INSTRUCTOR_ID%>${instructorPanel.index}"
      ...
      <div class="form-group">
        ...
        <div class="col-sm-9">
          <input class="form-control" type="text" name="<%=ParamNameConst.ParamsNames.INSTRUCTOR_NAME%>"
                 id="<%=ParamNameConst.ParamsNames.INSTRUCTOR_NAME%>${instructorPanel.index}" value="${instructorPanel.instructor.name}"
      ...
      <div class="form-group">
        ...
        <div class="col-sm-9">
          <input class="form-control" type="text" name="<%=ParamNameConst.ParamsNames.INSTRUCTOR_EMAIL%>"
                 id="<%=ParamNameConst.ParamsNames.INSTRUCTOR_EMAIL%>${instructorPanel.index}" value="${instructorPanel.instructor.email}"
      ...
      <div class="form-group">
        <label class="col-sm-3 control-label">
          <input type="checkbox" name="<%=ParamNameConst.ParamsNames.INSTRUCTOR_IS_DISPLAYED_TO_STUDENT%>" value="true"
