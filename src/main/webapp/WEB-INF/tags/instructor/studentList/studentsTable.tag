...
<div class="panel ${PANEL_TYPE}">
  <div class="panel-heading ajax_submit">
    <form style="display:none;"
      ...
      <input type="hidden" name="<%= ParamNameConst.ParamsNames.COURSE_ID %>" value="${course.courseId}">
      <input type="hidden" name="<%= ParamNameConst.ParamsNames.USER_ID %>" value="${course.googleId}">
    ...
    <a class="btn btn-info btn-xs pull-right pull-down course-enroll-for-test"
        ...
        title="<%= ToolTipConst.Tooltips.COURSE_ENROLL %>"
        data-toggle="tooltip"
        data-placement="top"
