...
<form method="post" name="form_submit_response" action="${data.submitAction}">
  <input type="hidden" name="<%= ParamNameConst.ParamsNames.FEEDBACK_SESSION_NAME %>" value="${data.bundle.feedbackSession.feedbackSessionName}">
  <input type="hidden" name="<%= ParamNameConst.ParamsNames.COURSE_ID %>" value="${data.bundle.feedbackSession.courseId}">
  ...
  <c:choose>
    ...
    <c:otherwise>
      <input type="hidden" name="<%=ParamNameConst.ParamsNames.REGKEY %>" value="${data.encryptedRegkey}">
      <input type="hidden" name="<%=ParamNameConst.ParamsNames.STUDENT_EMAIL %>" value="${data.account.email}">
    </c:otherwise>
  </c:choose>
    ...
    <c:choose>
      ...
      <c:otherwise>
        ...
        <button type="submit" class="btn btn-primary center-block margin-top-7px"
            ...
            data-placement="top" title="<%= ToolTipConst.Tooltips.FEEDBACK_SESSION_EDIT_SAVE %>"
